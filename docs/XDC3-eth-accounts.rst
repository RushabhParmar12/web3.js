...
:ref:`xdc3.eth.personal.newAccount() <personal-newaccount>` which creates an account
...
.. code-block:: javascript
    ...
    xdc3.eth.accounts.privateKeyToAccount(privateKey);
...
1. ``tx`` - ``Object``: The transaction's properties object as follows:
    ...
    - ``gasPrice`` - ``String``: (optional) The gas price set by this transaction, if empty, it will use :ref:`xdc3.eth.gasPrice() <eth-gasprice>`
...
2. ``keyName`` - ``String``: (optional) The key used for the local storage position, defaults to ``"xdc3js_wallet"``.
...
2. ``keyName`` - ``String``: (optional) The key used for the localstorage position, defaults to ``"xdc3js_wallet"``.
